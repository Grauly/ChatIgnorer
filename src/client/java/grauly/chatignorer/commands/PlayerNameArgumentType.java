...
public class PlayerNameArgumentType implements ArgumentType<String> {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        CommandSource.suggestMatching(getPlayerNames(source), builder);
        CommandSource.suggestMatching(AutoConfig.getConfigHolder(ChatIgnorerConfig.class).getConfig().ignoredPlayers, builder);
        return builder.buildFuture();
    }

    private List<String> getPlayerNames(FabricClientCommandSource source) {
        return source.getClient().getNetworkHandler().getListedPlayerListEntries().stream().map(p -> p.getDisplayName().getString()).map(p -> p.replaceAll("\\[.*\\] ", "")).toList();
    }
}
